...
agent runs `git init` for a project, whether on create or through Initialize
Git, it also writes a default `.gitignore` if the project has none. From the grace period task, the
