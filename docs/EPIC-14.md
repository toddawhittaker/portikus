...
1. **Does any site need more than one Entra tenant?** Ruling 8 supports one. Several means our own issuer check on Entra's shared endpoint, about two more days and a security review.
2. **Retire the users file after the import (ruling 23)?** The recommendation is yes: one place for Dex passwords, changed from the admin area. The cost is that `make users-add` and its encrypted copy in the nightly backup go away; the `dex` database dump replaces the copy.
