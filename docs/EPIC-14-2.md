...
Only two, and the brief is buildable as written without an answer to either.

1. **Entra roles: security groups through Dex's `microsoft` connector, or app roles through Dex's generic `oidc` connector?** ADR 0031 chose the first and says Portikus cannot read Entra app roles. Research shows it can: Dex's generic `oidc` connector pointed at the tenant's own issuer reads any ID token claim as groups (`insecureEnableGroups` with `claimMapping.groups: roles`), so the app roles Epic 14 used (`Portikus.Student`, `Portikus.Instructor`, `Portikus.Administrator`) pass straight through Dex. The `microsoft` connector, by contrast, can only fetch groups with the delegated Microsoft Graph permission `Directory.Read.All`, which it hard-codes and which needs a tenant administrator's consent. That is a broad permission (read the whole directory as the signed-in person) that a campus IT office may refuse. **Recommendation: use the `oidc` connector for Entra.** It needs no Graph permission and no `graph.microsoft.com` egress, it keeps Epic 14's Entra setup steps almost unchanged, and it is the connector CI tests end to end against the mock provider (ruling 13). This brief is written for the ADR as accepted (the `microsoft` connector, ruling 5). If Todd switches, ruling 5 becomes "`portikus_dex_upstream: entra` renders an `oidc` connector with the tenant issuer and `roles` as the groups claim", `microsoft` leaves the list, the connector also needs `insecureSkipEmailVerified: true` (Entra sends no `email_verified`), and T3 gets smaller by about half a day.
2. **Should Add user and Reset password in the Users view also make the person change the password at first sign-in?** The administrator has seen that password, so forcing a change would be sensible, and the flag this epic adds makes it two lines. It was not asked for, so ruling 17 leaves it out. **Recommendation: yes, in this epic.**
...
- **Entra ID**: Microsoft's identity service. A **tenant** is one organisation in it. A **security group** is an Entra group used for access control; each group has a permanent **object ID** (a GUID) and a **display name** anyone allowed to create groups can copy. An **app role** is a role defined on an app registration and assigned to people; it arrives in the `roles` claim.
- **Microsoft Graph**: Microsoft's web API (`graph.microsoft.com`). Dex's `microsoft` connector calls it to read the person and their groups.
...
- Under Entra, people from the site's tenant sign in; their role comes from Entra security groups the site names, or from grants in the Users view.
...
- Anyone with a Dex local password can change it from Settings, Password.
...
1. **(user)** ADR 0031 in full. Portikus signs people in through Dex only, plus LTI. Each institution provider is one Dex connector: `microsoft`, `google`, `ldap` or `oidc`. Dex's local passwords are always on. The API's direct Entra, Google and generic OIDC paths are removed; its one issuer is Dex.
...
5. **(user, with brief details)** **Entra through Dex's `microsoft` connector, with roles from security groups or from grants.** Two shapes, chosen by whether the site names groups:
    - **No groups named:** today's connector, restricted by `tenant`. Everyone in the tenant who can get a token signs in as a student; roles come from grants, and the API reads no groups claim, as today. "Assignment required" on Dex's app registration in Entra is the recommended gate, as it was for Epic 14's direct Entra.
    - **Groups named** (`portikus_entra_student_group`, `portikus_entra_instructor_group`, `portikus_entra_admin_group`, each an object ID): the connector renders `groups` set to those IDs, `useGroupsAsWhitelist: true`, `groupNameFormat: id` and `onlySecurityGroups: true`. Dex then refuses anyone in none of them, and passes on only the matching IDs; the API's `OIDC_STUDENT_GROUP`, `OIDC_INSTRUCTOR_GROUP` and `OIDC_ADMIN_GROUP` are the same three IDs, and the API asks for the `groups` scope.

    **(brief)** Groups are matched by **object ID, never by display name.** Dex's default is the display name, and by default any Entra user may create a security group, so a student could make one named like the administrators' group. That is the reason the play today refuses the `groups` scope behind this connector; with IDs the refusal is lifted for `microsoft` only. The connector asks Microsoft for `Directory.Read.All` (delegated) whenever groups are named; the tenant administrator must grant consent for it. It calls `graph.microsoft.com` for `/v1.0/me` in both shapes, so that host is already on the egress list (through the tenant's discovery document) and stays there; ruling 21 names it explicitly.
6. **(brief)** **Admission moves into Dex, per connector:**
    ...
    | `microsoft`, no groups | the tenant; "Assignment required" recommended | nothing (grants) | student |
    | `microsoft`, groups | members of one of the three groups (Dex refuses the rest) | the group IDs | refused by Dex |
...
16. **(orchestrator)** **Settings, Password, for every Dex local account.** The current password is checked through Dex's gRPC `VerifyPassword`, and the new hash is written with `UpdatePassword`; both exist in the pinned `api.proto` (`packages/auth/proto/dex-api.proto`), and the client gains one method, `verifyPassword`. While the flag is set, the same form is the only page the local administrator can reach. **(brief)** This is a small addition (about a day across server and web), because the flag needs the form anyway; it is not narrowed.
...
17. **(brief)** **Only the local administrator gets the flag.** Add user and Reset password in the Users view keep Epic 14's behaviour (open question 2). Reason: not asked for, and it changes a flow people already use.
...
- `portikus_dex_upstream`: `none`, `ldap`, `microsoft`, `google`, `oidc`.
- New: `portikus_admin_email`; `portikus_entra_student_group`, `portikus_entra_instructor_group`, `portikus_entra_admin_group` (object IDs, all three or none); `portikus_dex_upstream_issuer`, `portikus_oidc_upstream_groups_claim`, `portikus_oidc_upstream_extra_scopes` (for `oidc`).
...
- The three `PORTIKUS_OIDC_*_GROUP` inputs stay, for the `ldap` and `oidc` connectors' group names; under `microsoft` they are set from the three object IDs.
...
3. **An Entra sign-in with groups.** Dex sends the person to the tenant; Microsoft asks for their second factor under the tenant's rules; Dex reads their security groups from Graph, refuses them if none of the three IDs match, and otherwise hands Portikus a token whose `groups` holds the matching IDs; `mapRole` maps them.
...
5. **Changing a Dex password.** Settings, Password: current password, new password twice. The API checks the current one with Dex and stores the new hash in Dex; other sessions end.
...
- Under `microsoft` with groups, a group whose display name matches an administrator group's name but whose ID does not grants nothing.
...
| **T2 Local administrator and password change (server)** | builder | `packages/db/src/migrations/0019_local_admin.ts`, `migrations/index.ts`, `schema.ts`, `db.test.ts`, `testing.ts`; `packages/auth/src/local-admin.ts`, `reset-admin-main.ts` (new) and tests; `dex-api.ts` (`verifyPassword`) and its tests; `testing/fake-dex-grpc.ts`; `sessions.ts`, `plugin.ts`, `index.ts` and tests; remove `setup-code.ts`, `setup-code-main.ts`, `setup-code.test.ts`; `packages/contracts/src/auth.ts`, `index.ts`, remove `setup.ts`; `apps/api/src/routes/me-password.ts` (new) and test, remove `routes/setup.ts` and test; `apps/api/src/routes/auth.ts` and test (`/auth/me` fields only, after T1); `apps/api/src/routes/preview.ts` (the gate only); `apps/api/src/signin-throttle.ts`; `apps/api/src/server.ts`, `security/route-policy.ts`, `authz-matrix.test.ts` | T1 (shares `auth.ts`, `types.ts`, `index.ts`) | 2 days |
| **T3 Dex connectors, the play and the command** | infra | `infra/ansible/site.yml`; `infra/ansible/roles/dex/**` (the `oidc` connector, `microsoft` groups, settings guards); `infra/ansible/roles/portikus/**` (api.env, remove the setup-code tasks, run `portikus reset-admin --if-missing`, remove the stale file); `infra/ansible/roles/egress_proxy/**` only if ruling 21 needs it; `infra/ansible/roles/caddy/templates/Caddyfile.j2` (remove `handle /setup/*`); `packaging/bin/portikus` (new), `packaging/nfpm.yaml`, `scripts/build-deb.sh` if it lists shipped files; `infra/tests/dex-render-test.yml` and fixtures, `egress-proxy-render-test.yml`, `caddy-preview-test.sh`, `smoke-test.sh`, `security-test.sh`, `security/lib.sh`, `rebuild-exercise.sh` (the `PORTIKUS_IDP` values); `Makefile` (help text, the rehearsal comment) | T2 for the rehearsal run only | 2 days |
| **T4 Web and Playwright** | builder, then tester for e2e | `apps/web/src/password/**` (new: the change-password page and the Settings section's form); `apps/web/src/settings/sections.ts`, `SettingsDialog.tsx` and tests; `apps/web/src/router.tsx` and test; `apps/web/src/useMe.ts`; remove `apps/web/src/setup/**`; `apps/web/vite.config.ts` (remove the `/setup/` proxy); `e2e/fake-dex-grpc.mjs` (`VerifyPassword`); `e2e/change-password.spec.ts`, `e2e/a11y-change-password.spec.ts` (new); remove `e2e/setup.spec.ts`, `e2e/a11y-setup.spec.ts` | T2 | 1.5 days |
| **T5 CI against the real Dex** | tester | `.github/workflows/ci.yml` (the `dex-signin` job only); `packages/auth/src/dex.integration.test.ts`, `dex-api.integration.test.ts`, `testing/dex-signin.ts`; a new `infra/tests/fixtures` entry for the `oidc` render | T2, T3 | 1 day |
| **T6 Docs, rehearsal and pilot** | builder for docs, infra for the rehearsal and pilot | `docs/SPEC.md` (5.1, 5.2, 5.3, 24.11, 29); `docs/EPIC-14.md` (a superseded note per ruling); `docs/adr/0031` (status, and "prints the password once" corrected to the file), `0028` and `0023` (status lines); `docs/OPERATIONS.md` (every provider section, "The first administrator" becomes "The local administrator"); `infra/README.md`; `docs/OVERVIEW.md`; `docs/STATUS.md`; `docs/BACKLOG.md` | all others | 1.5 days docs, 1 day rehearsal and pilot |

T1, and T3's connector and render work, start together. T2 starts when T1 lands. T4 starts when T2 lands; T5 when T2 and T3 have. T6 is last. Total work is about 10 agent-days; with the parallel starts it is about 6 working days end to end, a little over the BACKLOG's "about a week".
...
- Dex's `microsoft` connector against a real tenant: the `Directory.Read.All` consent, `getMemberGroups` returning transitive security-group IDs, and a refusal for a person in none of the groups. The connector's `apiURL` and `graphURL` options would let a fake Graph test it in CI; that is left out.
...
3. **`Directory.Read.All` may be refused by a campus IT office.** Then the site runs `microsoft` without groups and uses grants, or Todd takes open question 1's alternative.
...
- Forcing a password change after Add user or Reset password (open question 2).
- Two-factor sign-in for Dex local passwords (Dex has none at the pinned version; ADR 0031).
- A fake Microsoft Graph for CI, and Google groups through the Admin SDK.
