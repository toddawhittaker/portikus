...
- Acceptance is a gate on the same mechanism as "must change password" (section 5.3). The gates are one ordered list in `packages/auth`, and the first unmet gate wins:
  1. `must_change_password` is set: code `PASSWORD_CHANGE_REQUIRED`, extra allowed route `POST /me/password`, web page `/change-password`;
  2. the account's accepted version differs from the current one: code `ACCEPTABLE_USE_REQUIRED`, extra allowed routes `GET /acceptable-use` and `POST /me/acceptable-use`, web page `/acceptable-use`.

  So a new local administrator changes the password first, then accepts. While a gate is unmet, every other API route answers 403 with that gate's code, except the routes every gate allows (`GET /auth/me`, `POST /auth/logout` and the routes that need no session); WebSocket upgrades and the preview gateway refuse the account too. `loadSession` computes both gates in its one query.
- `GET /auth/me` carries `mustAcceptUse: boolean`, and the web sends every page to the first unmet gate. The page shows the text, **I accept** and **Sign out**.
- `POST /me/acceptable-use {version}` is CSRF-checked. It answers 409 `ACCEPTABLE_USE_CHANGED` and records nothing when `version` is not the current one, so nobody accepts a text they did not see. Accepting is audited as `user.acceptable_use_accepted`.
- The gate is checked on every request, so a text change reaches people already signed in at their next request. Their workspaces keep running.
...
- CPU is judged across runs over a rolling window of wall-clock time, and usage is remembered across stops and restarts. The anchor is the newest sample at least one window old; with no such sample there is no decision yet. The CPU time used between each pair of consecutive samples from the anchor to now is summed. Across a restart, the later sample's whole counter counts, plus the time between the two samples, capped at one sample interval (60 seconds), times the CPU limit, as if the workspace had used every CPU before restarting. Stopped time has no samples and counts as no use. The average is the used CPU time divided by the time since the anchor times the CPU limit. So a student who runs 25 minutes and stops for one, over and over, is still throttled, while an honest restart in a quiet window adds at most one minute of assumed use.
...
- A workspace whose CPU average is above the CPU threshold (default 80%) over the window (default 30 minutes) is throttled to the throttle share (default 25%) of its CPU limit. The throttle is Incus's `limits.cpu.allowance` written as a time slice, `<N>ms/100ms`, where N is the share times the CPU limit times 100 ms, rounded to a whole millisecond: 25% of 4 CPUs is `100ms/100ms`, one CPU's worth. It is never a percentage, which Incus treats as a soft weight that only applies when the host is busy.
