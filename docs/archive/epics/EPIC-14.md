...
**Later decision (Todd, 2026-09-25), built by Epic 14.2:** ADR 0031 makes Dex the only sign-in front door and replaces the setup code with a local administrator made at install. The rulings marked "Superseded by Epic 14.2" below no longer hold; SPEC.md sections 5.1 to 5.3 and 24.11 give the current rules. This brief still describes what Epic 14 built.
...
2. **Superseded by Epic 14.2** (Entra is now a Dex connector held to the tenant's issuer, with roles from app roles; the API has no `tid` check.) **(user)** Microsoft Entra ID by direct OIDC; sign-in restricted to the allowed tenant by the `tid` claim; roles from Entra app roles in the `roles` claim.
3. **Superseded by Epic 14.2** (Google is now Dex's `google` connector, restricted by `hostedDomains`; the API has no `hd` check.) **(user)** Google Workspace by direct OIDC; sign-in restricted by the `hd` claim; roles from Portikus grants, the `granted_role` of Epic 13.1, because Google sends no groups.
...
6. **Superseded by Epic 14.2** (Dex's generic `oidc` connector is now built, and admits only members of the three groups.) **(user)** Generic OIDC (Okta, Keycloak, Shibboleth's OIDC plugin): configuration and docs only.
...
8. **Superseded by Epic 14.2** (the one tenant is now enforced by Dex's `entra` connector through the tenant's own issuer, not by `tid`.) **(brief)** **One tenant per Entra site.** The issuer is the tenant's own, `https://login.microsoftonline.com/<tenant ID>/v2.0`, and `tid` must equal that tenant ID. Several tenants would need Entra's shared `organizations` endpoint, whose ID tokens carry a different issuer per tenant; `openid-client` checks the issuer against one value, so accepting several means replacing that check with our own. That is security-sensitive code for a need nobody has yet. **(Todd, 2026-09-24)** One Entra tenant per site, as ruled. Multi-tenant support is left out; if it is ever needed it is about two days of work plus a security review. Guests invited into the tenant (Entra B2B) sign in and are checked like anyone else.
9. **Superseded by Epic 14.2** (the API reads neither `tid` nor `hd`; Dex refuses people before Portikus sees them.) **(brief)** **Admission is checked on the ID token's claims only.** The `tid` and `hd` checks read the signed ID token, never userinfo. A missing claim is a refusal. A refused sign-in gets the existing `not_authorized` page and an `auth.login` `denied` audit row with `{reason: "tenant_not_allowed"}` or `{reason: "domain_not_allowed"}`.
10. **Superseded by Epic 14.2** (the API always calls userinfo, which is Dex's; Dex's `entra` connector skips Entra's userinfo.) **(brief)** **No userinfo call for Entra and Google.** Their ID tokens carry `sub`, `name`, `email` and the admission and role claims. Skipping userinfo removes a second host from each allow list (`graph.microsoft.com`, `openidconnect.googleapis.com`) and a failure point. Dex and generic OIDC keep the userinfo call, because Dex puts `groups` there.
11. **Superseded by Epic 14.2** (see the connector table in SPEC.md section 5.1; `OIDC_DEFAULT_ROLE` is `student` on every Dex site.) **(brief)** **Where the role comes from, per provider.** One new setting, `OIDC_DEFAULT_ROLE`, `none` or `student`, says what an admitted person gets when no group or app role matches. `mapRole` is otherwise unchanged: Entra is the existing groups mapping with the claim `roles` and the app role values `Portikus.Student`, `Portikus.Instructor` and `Portikus.Administrator`.
...
13. **Superseded by Epic 14.2** (in part: Entra now uses Dex's `oidc` connector, not `microsoft`, and the generic `oidc` connector is built.) **(brief)** **Dex in front of Entra or Google uses Dex's own `microsoft` or `google` connector**, restricted by its `tenant` or `hostedDomains` option, beside local passwords for guests. Only one upstream connector per site. Dex's generic OIDC connector is left out.
...
- Dex's SAML and generic OIDC connectors; more than one upstream connector. (Epic 14.2 built the generic OIDC connector.)
- Self-service password change or reset for Dex users; e-mailed invitations. (Epic 14.2 built password change in Settings, Password.)
