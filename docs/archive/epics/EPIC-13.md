...
4. **Roles.**
   ...
   - The Dex users file also accepts `instructor`: `make users-add` offers it, `users-check` accepts it, the Dex rendering gives it the group `portikus-instructors` (amended in the confirmation review), and `mapRole` maps the new `OIDC_INSTRUCTOR_GROUP` (default `portikus-instructors`, amended after review; Ansible `portikus_oidc_instructor_group`) to it. `make users-deploy` ends a user's sessions on any role change (amended after review).
...
10. **Tests.** Unit tests for validation including every defect; database tests; Playwright end-to-end tests for a student launch, an instructor launch and the Course page, the frame fallback page, and administrator routes refused for an instructor; a smoke-test block and security-test checks on a real VM; the LTI specs run in the ordinary end-to-end shards, with no separate CI job (amended in the confirmation review, for CI speed); accessibility of the Course page and the fallback page (SPEC.md section 25.8).
...
17. **Frames.** Caddy today sends `frame-ancestors 'none'` on every control-plane response, and that stays for everything except `/lti/*`.
    ...
    - (Amended in the confirmation review:) `/lti/login` writes the state row and sets its cookie only when `Sec-Fetch-Dest` is `document` or absent. Any other value (`image`, `script`, `empty`, and so on) gets the 400 refusal page, logged but not audited, so a page cannot pile up state cookies by firing images at it until the browser evicts the session cookie. A request whose `Sec-Purpose` names `prefetch` or `prerender`, or that carries `Purpose: prefetch`, is refused the same way, because a speculative prefetch also says `document`. A new login also clears the oldest `__Host-portikus_lti_state_*` cookies the request carries, so at most four remain besides the new one.
...
- `make mock-lms` runs it on the host in the foreground, bound to `127.0.0.1` and `10.100.0.1` by default (`MOCK_LMS_BIND` overrides), so nobody on the LAN can launch as anyone. (Amended after the pilot: it now also binds and registers at the host's LAN address, `HOST_IP`, because the user's browser follows the login redirect to it and cannot reach `10.100.0.1` from another machine.) It is never installed on the VM.
...
| **T5 Mock LMS and browser tests** | tester, with a builder for the package | `packages/mock-lms/**` (new); `pnpm-lock.yaml`; `playwright.config.ts` (a web server for the mock); `e2e/lti-launch.spec.ts`, `e2e/lti-course.spec.ts`, `e2e/lti-frame.spec.ts`, `e2e/lti-negative.spec.ts`, `e2e/a11y-course.spec.ts` (axe on the Course page and both fallback pages); `.github/workflows/ci.yml` (no separate job; amended in the confirmation review) | T3 and T4 for the specs to pass; the package starts at once | No |
