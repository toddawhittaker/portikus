...
10. **(orchestrator)** **"Above 80% for 30 minutes" is an average over a rolling 30-minute window of the per-minute samples.** **(brief)** Each tick, for each running workspace:
    - CPU: take the newest sample at least `window` minutes old, taken since the workspace last started. The average is `(cpuUsageNs now − cpuUsageNs then) / (elapsed nanoseconds × cpuLimit)`, as a percentage. Because the counter is cumulative this is exact even if the worker missed some ticks.
    - Memory: the mean of `memoryBytes / memoryLimitBytes` over the samples in the window, and only when there are at least `window / 2` of them.
    - No sample old enough since the last start means no decision yet.
...
14. **(brief)** **The allowance is removed at every start.** The controller's start removes `limits.cpu.allowance` from the instance before starting it (a read with its ETag and a guarded write, as `putIfMatch` already does). The worker clears `cpu_throttle` and writes `workspace.cpu_throttle_lifted` with `{reason: "stopped"}` when it records the workspace stopped by any path. Reason: ruling 1's "lifted at the next stop and start" in one place on each side.
...
22. **(orchestrator)** **Activity is reported by the web app and recorded by the API; the workspace agent reports nothing.** **(brief)** What counts, all only from the workspace's owner, never from an administrator looking at it:
    ...
    - a page load in a preview: the edge check `/preview/authorize` sees `Sec-Fetch-Dest: document` (the header Caddy's `@app_page` matcher already uses; `forward_auth` passes the browser's headers on). Assets, fetches and a dev server's reload socket do not count.
...
- That Caddy's `forward_auth` passes `Sec-Fetch-Dest` to `/preview/authorize` (T4 checks it on the rehearsal VM; if it does not, a preview visit counts on any authorized request that accepts `text/html`).
