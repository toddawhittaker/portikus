...
10. **(orchestrator)** **"Above 80% for 30 minutes" is an average over a rolling 30-minute window of the per-minute samples.** **(brief)** Each tick, for each running workspace:
    ...
    - **(orchestrator, security confirmation review)** A restart between two consecutive samples is a changed boot marker. When either sample has no marker, it falls back to the CPU counter dropping. A reboot from inside the workspace (students have passwordless sudo) zeroes the counter and changes the marker, so it is always seen, even when the new counter happens to be above the old one.
    - CPU is judged across runs: take the newest sample at least `window` minutes old, stopped time included (the anchor). Sum the CPU time used between each pair of consecutive samples from the anchor to now. Across a restart, the later sample's whole value counts, plus the time between the two samples, capped at one sample interval (60 seconds), times `cpuLimit`: the unseen use before the restart is assumed to be full use. Otherwise a unit that burns every CPU for 55 seconds and reboots would read about 45% while really using about 90%. Time the workspace was stopped has no samples and so counts as no use, apart from that one-interval allowance. The average is `used CPU time / (nanoseconds since the anchor × cpuLimit)`, as a percentage. A student who runs 25 minutes and stops for one, over and over, is throttled; an honest restart in a quiet window adds at most one minute of assumed use and is not.
    - **(orchestrator, confirmation review)** Memory is judged per run: the mean of `memoryBytes / memoryLimitBytes` over the samples in the last window of wall-clock time taken since the latest restart, and only when there are at least `window / 2` of them. For memory, a gap of more than two sample intervals also counts as a restart, since a stop leaves no samples. So a flag cleared at a stop does not come back from memory the workspace no longer holds; memory high again for half a window in the new run is flagged again. The samples themselves are kept, so the CPU history survives.
...
3. **A student can lift their own throttle** by stopping and starting (ruling 1), and the restart then starts a fresh window (ruling 11). Stopping and starting before a throttle does not help, and neither does a reboot from inside the workspace, which the boot marker detects (ruling 10, security confirmation review): usage is remembered across restarts (ruling 10, Todd 2026-09-25), so a 25-minutes-on, 1-off cycle is still throttled. Each throttle writes an audit row, so an administrator sees a repeat offender in the detail panel's recent events.
