...
- Upgrades: an `apt upgrade` onto the Epic 14.2 release must run `portikus setup` (the play), because only the play reissues Dex's gRPC certificate with `localhost` and removes the retired direct-provider settings from api.env (docs/OPERATIONS.md, "Deploying"). The upgrade path this epic builds must do that, or handle the certificate reissue itself.
