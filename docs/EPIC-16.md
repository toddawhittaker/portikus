...
4. **No unit sets `OOMScoreAdjust`**; the agent and the terminals stay at 0, like the student's Docker containers. Reason: the audit's suggested +300 would make terminal programs preferred victims over Docker containers for no stated gain. **(orchestrator, after the rehearsal)** The agent's planned `OOMScoreAdjust=-500` was dropped: the kernel refuses to lower the score in an unprivileged container, so the real value stayed 0.
...
8. **The exit record.** The terminals unit sets `RuntimeDirectory=portikus-terminals`, `RuntimeDirectoryMode=0755`, `RuntimeDirectoryPreserve=yes`, and `ExecStopPost=/usr/local/sbin/portikus-terminals-exit`, which writes `$SERVICE_RESULT` to `/run/portikus-terminals/last-exit`, or `oom-kill` when the exit status is `KILL` and the unit's cgroup `memory.events` shows an `oom_kill`. **(orchestrator, after the rehearsal)** With `OOMPolicy=continue` systemd reports an out-of-memory kill of tmux as `signal`, so `$SERVICE_RESULT` alone never said `oom-kill`. The file's modification time is when it stopped. `/run` is cleared when the workspace stops, so any record present means the terminals restarted during this boot. The student can write the file; the only effect is their own message.
...
18. **(withdrawn after the rehearsal)** Checks were to run under `choom -n 0` so they would not inherit the agent's -500. With the -500 gone (ruling 4) there is nothing to undo, so Checks run `bash -lc` directly.
...
|---|---|---|---|---|---|---|
| **T1 Image and units** | Rulings 1 to 8: `/tmp` and `/dev/shm` caps; the terminals unit; the agent unit's `TasksMax`, `TMUX_EXTERNAL_SERVER` and ordering; the exit record; `VERSION` to 2026.09.11. Infra checks (below) and one rehearsal run. SPEC.md §19.3, §21.7. | #618, #619, #610 (unit side) | infra | `infra/workspace-image/portikus.yaml`, `infra/workspace-image/VERSION`; `infra/tests/smoke-test.sh`, `infra/tests/security/limits.sh`, `infra/tests/security/container.sh` | T2 for the rehearsal run only (writing can start at once) | 1.5 days |
| **T2 Terminals and process trees** | Rulings 11 to 18: socket name `portikus`, external mode with `-N`, `-f /dev/null`, tmux call timeout, the `portikus-shell` wrapper, process-tree stop for terminal close and Check stop, Checks backpressure. SPEC.md §9.7, §18.1. | #610 (agent side), #620, #624 | builder | `apps/workspace-agent/src/tmux.ts`, `terminals.ts`, `cwd.ts`, `checks-route.ts`, `process-tree.ts` (new), `server.ts`, `index.ts` and their tests; `apps/workspace-agent/scripts/portikus-shell` (new) and its test; `apps/workspace-agent/package.json` and `scripts/build-deb.sh` only as far as shipping the wrapper needs; `packages/config/src/index.ts` and test | none | 2.5 days |
...
- `systemctl show` gives `TasksMax=infinity` on the agent unit and `TasksMax=1700` on the terminals unit.
- The tmux server's cgroup is the terminals unit's.
